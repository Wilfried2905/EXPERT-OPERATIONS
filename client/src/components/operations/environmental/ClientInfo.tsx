...
export default function EnvironmentalAuditClientInfo() {
  ...
  const [clientInfo, setClientInfo] = useState({
    ...
    businessType: '',
    environmentalScope: '',
    existingEnvironmentalCertifications: '',
    additionalNotes: ''
  });
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <h1 className="text-2xl font-bold">{t('environmental.clientInfo.title')}</h1>
      <p className="text-gray-600">{t('environmental.clientInfo.description')}</p>
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="block font-medium">{t('environmental.clientInfo.environmentalScope')}</label>
              <textarea
                className="w-full p-2 border rounded-md"
                rows={3}
                value={clientInfo.environmentalScope}
                onChange={(e) => handleInputChange('environmentalScope', e.target.value)}
              />
            </div>

            <div className="space-y-2">
              <label className="block font-medium">{t('environmental.clientInfo.existingEnvironmentalCertifications')}</label>
              <textarea
                className="w-full p-2 border rounded-md"
                rows={3}
                value={clientInfo.existingEnvironmentalCertifications}
                onChange={(e) => handleInputChange('existingEnvironmentalCertifications', e.target.value)}
              />
            </div>
