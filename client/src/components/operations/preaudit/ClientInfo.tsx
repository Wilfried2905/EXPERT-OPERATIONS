...
export default function PreAuditClientInfo() {
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <h1 className="text-2xl font-bold">{t('preaudit.clientInfo.title')}</h1>
      <p className="text-gray-600">{t('preaudit.clientInfo.description')}</p>
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="block font-medium">{t('clientInfo.certificationScope')}</label>
              <textarea
                className="w-full p-2 border rounded-md"
                rows={3}
                value={clientInfo.certificationScope}
                onChange={(e) => handleInputChange('certificationScope', e.target.value)}
              />
            </div>

            <div className="space-y-2">
              <label className="block font-medium">{t('clientInfo.existingCertifications')}</label>
              <textarea
                className="w-full p-2 border rounded-md"
                rows={3}
                value={clientInfo.existingCertifications}
                onChange={(e) => handleInputChange('existingCertifications', e.target.value)}
              />
            </div>
